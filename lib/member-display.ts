...
/** Same ordering as family profile: head, spouse, child, other. */
 ...
 * When true: head + spouse as "First Last" joined with "and" (Oxford-style for 3+),
...
export function formatMemberDisplayLine(
  ...
  const adults = list.filter((m) => m.role === 'head' || m.role === 'spouse')
  const rest = list.filter((m) => m.role !== 'head' && m.role !== 'spouse')
