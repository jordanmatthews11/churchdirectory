...
const PAGE_BITMAP_JPEG_QUALITY = 0.92
  ...
  async function handleExportWebPdf() {
    ...
    try {
      ...
      for (let i = 0; i < canvases.length; i += 1) {
        ...
        canvas.width = 0
        canvas.height = 0
        ;(canvases as Array<HTMLCanvasElement | null>)[i] = null
      }
  ...
  async function handleExportBookPdf() {
    ...
    try {
      ...
      for (let s = 0; s < sheetCount; s += 1) {
        ...
        for (const idx of new Set([fl, fr, bl, br])) {
          const canvas = canvases[idx]
          if (!canvas) continue
          canvas.width = 0
          canvas.height = 0
          ;(canvases as Array<HTMLCanvasElement | null>)[idx] = null
        }
      }
